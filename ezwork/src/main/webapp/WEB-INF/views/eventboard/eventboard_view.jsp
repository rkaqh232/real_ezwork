...
<body>
	<div class="container">
				...
				<table class="table" >
					...
					<tbody style="text-align:center; font-size:14px;">
						...
						<tr>
							...
							<td><div>${boarddata.EV_TITLE } </div></td>
							<c:if test="${boarddata.EV_RE_LEV==0 }">
								<%--원문글인 경우에만 첨부파일을 추가 --%>
							<th class="table-Secondary"><div>첨부파일</div></th>
					<c:if test="${!empty boarddata.EV_FILE }"><%-- 파일첨부한 경우 --%>
						<%--파일첨부한 경우 --%>
						<td><img src="resources/image/down.png" width="10px"> 
						<a href="BoardFileDown.ev?filename=${boarddata.EV_FILE}&original=${boarddata.EV_ORIGINAL}">
								${boarddata.EV_FILE }</a></td>
					</c:if>
						<c:if test="${empty boarddata.EV_FILE }"><%--파일 첨부하지 않은 경우 --%>
						<%--파일 첨부하지 않은 경우 --%>
						<td>첨부파일 없음</td>
					</c:if>
					</c:if>
		...
		<!-- 수정 -->
		<a href="BoardModifyView.ev?num=${boarddata.EV_NO }" class="btn btn-default btn-icon btn-sm mr-2" >
		...
		<!-- 목록 -->
		 <a href="BoardList.ev" class="btn btn-default btn-icon btn-sm mr-2">
