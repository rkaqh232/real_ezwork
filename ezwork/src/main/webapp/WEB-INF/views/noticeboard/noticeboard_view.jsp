...
<body>
	<div class="container">
		...
		<div class="card card-custom">
			...
			<input type = "hidden" name="NO_NO" value ="${boarddata.NO_NO }">
			<input type = "hidden" name="NO_PASS" value ="${boarddata.NO_PASS }">
				...
				<table class="table" >
					...
					<tbody style="text-align:center; font-size:14px;">
						<tr>
							...
							<td><div>${boarddata.NO_SORT } </div></td>
							<th class="table-Secondary"><div>작성날짜</div></th>
							<td><div>${boarddata.NO_DATE }</div></td>
						</tr>
						<tr>
							...
							<td><div>
									<span
										...
										${boarddata.NO_NAME } </span>
								</div></td>
							<th class="table-Secondary"><div>조회수</div></th>
							<td><div>${boarddata.NO_READCOUNT } </div></td>
						</tr>
						<tr>
						</tr>
						<tr>
							<th class="table-Secondary"><div>제목</div></th>
							<td><div>${boarddata.NO_TITLE } </div></td>
							<c:if test="${boarddata.NO_RE_LEV==0 }">
								<%--원문글인 경우에만 첨부파일을 추가 --%>
							<th class="table-Secondary"><div>첨부파일</div></th>
					<c:if test="${!empty boarddata.NO_FILE }"><%-- 파일첨부한 경우 --%>
						<%--파일첨부한 경우 --%>
						<td><img src="resources/image/down.png" width="10px"> 
						<a href="BoardFileDown.no?filename=${boarddata.NO_FILE}&original=${boarddata.NO_ORIGINAL}">
								${boarddata.NO_FILE }</a></td>
					</c:if>
						<c:if test="${empty boarddata.NO_FILE }"><%--파일 첨부하지 않은 경우 --%>
						<%--파일 첨부하지 않은 경우 --%>
						<td>첨부파일 없음</td>
					</c:if>
					</c:if>
					...
					<textarea class="form-control" rows="5" readOnly
						style="height: 100%;">${boarddata.NO_CONTENT}</textarea>
					</div>
		...
		<!-- 수정 -->
		<a href="BoardModifyView.no?num=${boarddata.NO_NO }" class="btn btn-default btn-icon btn-sm mr-2" >
		...
		<!-- 목록 -->
		 <a href="notice.bo" class="btn btn-default btn-icon btn-sm mr-2">
					...
					<div class="modal-body">
						<form name="deleteForm" action="BoardDeleteAction.no"
							...
							<%--http://localhost:8088/Board_ajax_bootstrap/BoardDetailAction.bo?num=22
                				 ...
                				 --%>
							<input type="hidden" name="num" value="${boarddata.NO_NO}" id="board_num">
							<div class="form-group">
								<label for="pwd">비밀번호</label> <input type="password"
									class="form-control" placeholder="Enter password"
									name="NO_PASS" id="no_pass">
							</div>
